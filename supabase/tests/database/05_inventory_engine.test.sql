...
-- 1. Test add_stock function (positive)
SELECT lives_ok(
  $$ SELECT public.add_stock('10000000-0000-0000-0000-000000000009', 100, CURRENT_DATE + 30, CURRENT_DATE, '00000000-0000-0000-0000-000000000001'::uuid, 'Initial delivery') $$,
...
-- 4. Test add_stock function (negative quantity)
SELECT throws_ok(
  $$ SELECT public.add_stock('10000000-0000-0000-0000-000000000009', -50, CURRENT_DATE + 30, CURRENT_DATE, '00000000-0000-0000-0000-000000000001'::uuid, 'Bad delivery') $$,
  'Quantity must be greater than 0',
  'add_stock should throw exception on negative quantity'
);

-- 5. Test adjust_stock function (positive)
...
BEGIN
  ...
  PERFORM public.adjust_stock(v_batch_id, 80, '00000000-0000-0000-0000-000000000001'::uuid, 'Damaged items');
END $$;
...
-- 8. Test adjust_stock (missing reason)
SELECT throws_ok(
  $$ 
    DO $body$ 
    DECLARE v_batch_id UUID; 
    BEGIN 
      SELECT id INTO v_batch_id FROM public.stock_batches WHERE item_id = '10000000-0000-0000-0000-000000000009' LIMIT 1; 
      PERFORM public.adjust_stock(v_batch_id, 70, '00000000-0000-0000-0000-000000000001'::uuid, ''); 
    END $body$; 
  $$,
  'Reason is required for stock adjustments',
  'adjust_stock should require a reason'
);

-- 9. Test adjust_stock (negative quantity)
SELECT throws_ok(
  $$ 
    DO $body$ 
    DECLARE v_batch_id UUID; 
    BEGIN 
      SELECT id INTO v_batch_id FROM public.stock_batches WHERE item_id = '10000000-0000-0000-0000-000000000009' LIMIT 1; 
      PERFORM public.adjust_stock(v_batch_id, -5, '00000000-0000-0000-0000-000000000001'::uuid, 'Invalid'); 
    END $body$; 
  $$,
